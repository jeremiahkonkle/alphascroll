                ...
                function createScrollbar() {
                    ...
                    alphascroll = $(self).closest('div').children('.alphascroll').addClass("ui-alphascroll-" + (selfdatatheme !== undefined && selfdatatheme !== '' ? selfdatatheme : 'a'));
                      ...
                      $(document).off('scroll').on('scroll', function (event) {
                          var  headerheight = $(self).closest(':jqmData(role=page)').find(':jqmData(role=header):first').outerHeight(),
                          scrolltop = $(this).scrollTop() >= 0 ? $(this).scrollTop() : 0;
