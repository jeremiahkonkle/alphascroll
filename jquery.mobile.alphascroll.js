    ...
    $.widget("mobile.listview", $.mobile.listview, {
        ...
        _displayList: function () {
            ...
            function createScrollbar () {
                ...
                if (!$(self).closest(':jqmData(role=page)').find(':jqmData(role=header):first').is(":jqmData(position=fixed)")) {
                    $(document).off('scroll').on('scroll', function (event) {

                        var headerheight = $(self).closest(':jqmData(role=page)').find(':jqmData(role=header):first').outerHeight(), scrolltop = $(this).scrollTop() >= 0 ? $(this).scrollTop() : 0;
                        if (scrolltop >= headerheight) {
                            $(alphascroll).css('top', 0);
                        }
                        else {
                            $(alphascroll).css('top', headerheight - scrolltop);
                        }
